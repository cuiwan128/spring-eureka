...
public class MovieController {
    ...
    private static Logger logger = Logger.getLogger("MovieController");
    ...
    @GetMapping("movie/{id}")
    public User finByIdRibbonMovie(@PathVariable Long id){
    ...
    @GetMapping("userHystrix/{id}")
    public User finByIdHystrix(@PathVariable Long id){
        String urlHystrix = userServerName+id;
        return this.restTemplate.getForObject(urlHystrix,User.class);
    }

    public User findByIdHystrixFallBack(Long id,Throwable throwable){
        User user = new User();
        user.setId(id);
        user.setUsername("出问题了");
